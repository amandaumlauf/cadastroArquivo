/*Autor: Amanda Umlauf e João Victor Campregher
Data:05/02/2022
Função: Um programa em C que realiza a gravação dos dados em um arquivo texto. 
Esse é um sistema de atendimento de uma autoescola, o usuário insere seus dados e informações referente a idade, caso seja maior de 18 anos, seus dados são salvos no arquivo e podem ser consultados pelos atendentes. 
*/
...
struct cadastro //Programação C - Structs. Structs, também conhecidas como Registros, definem tipos de dados que agrupam variáveis sob um mesmo tipo de dado. A ideia de usar uma struct é permitir que, ao armazenar os dados de uma mesma entidade, isto possa ser feito com uma única variável.
{
	char nome[50];
    int cpf;
	int telefone;
...
int main(){
	setlocale(LC_ALL, "Portuguese"); //coloca a linguagem em portugues, permite acentos
        ...
        switch (opcao) {//cria um sitch, cada um possuindo uma função diferente que pode 
